 ...
 */
enum class CurTransferFilterParams(
    override var field: String,
    override var fieldDataProvider: (CurTransferDto) -> Any
) : IFilterParams<CurTransferDto> {
    ...
    RECEIVE_DATE("receiveDate", { it.receiveDate ?: LocalDate.MIN }),                   //5.1
    ...
    EXECUTION_DATE("executionDate", { it.executionDate ?: LocalDate.MIN }),             //6.1
    ...
    VALUE_DATE("valueDate", { it.valueDate ?: LocalDate.MIN }),                                         //1.4
...
//    CUR_REGULATION_INFO_AMOUNT("regulationInfos.amount", { curTr -> curTr.regulationInfos.map { it.amount } }),             //5
    ...
    IS_RECEIVED("isReceived", fieldDataProvider = { it.isReceived ?: false }),
    IS_EXECUTED("isExecuted", fieldDataProvider = { it.isExecuted ?: false }),
    IS_DECLINED("isDeclined", fieldDataProvider = { it.isDeclined ?: false }),
    IS_NOT_RECEIVED("isNotReceived", fieldDataProvider = { it.isNotReceived ?: false }),
    IS_RECALLED("isRecalled", fieldDataProvider = { it.isRecalled ?: false }),
    ;
