 ...
 */
enum class PublicationSortingParams(
    override var field: String,
    override var fieldDataProvider: (PublicationDto) -> Any?
) : ISortingParams<PublicationDto> {
    ...
    IS_MUST_KNOW("isMustKnow", fieldDataProvider = { it.isMustKnow }),                  //1.8
    IS_IMPORTANT("isImportant", fieldDataProvider = { it.isImportant }),                //1.9
    START_AT("startAt", fieldDataProvider = { it.startAt }),                            //1.10
    END_AT("endAt", fieldDataProvider = { it.endAt }),                                  //1.11
    MAILING_CHANNEL("mailingChannel", fieldDataProvider = { it.mailingChannel }),       //1.12
    IS_FOR_ALL_BRANCHES("isForAllBranches", fieldDataProvider = { it.isForAllBranches }),   //1.13
    ...
    POSTED_AT("postedAt", fieldDataProvider = { it.postedAt }),                         //5.3
